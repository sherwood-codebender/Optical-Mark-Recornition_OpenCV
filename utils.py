# -*- coding: utf-8 -*-
"""
Created on Tue Aug 24 21:19:09 2021
...
def rectContour(contours):
    ...
    for i in contours:
        ...
        if area > 50:
            peri = cv2.arcLength(i, True)
            approx = cv2.approxPolyDP(i, 0.02*peri, closed=True)#[x, y] of corner point
            # print(len(approx))
            if len(approx) == 4:
                rectCon.append(i)
    rectCon = sorted(rectCon, key=cv2.contourArea, reverse=True)
...
def getCornerPoints(cont):
    ...
    approx = cv2.approxPolyDP(cont, 0.02*peri, closed=True)
    return approx

def reoder(myPoints):
    
    myPoints = myPoints.reshape((4,2)) #reshape to a matrix (row: x y)
...
def showAnswers(img, myIndex, grading, answer, questions, choices):
    ...
    for i in range(0, questions):
        ...
        if grading[i] == 1:
            ...
            myColor = (0, 0, 255)
            cv2.circle(img, (answer[i]*secW + secW//2, i*secH + secH//2), 20, (0, 255, 0), cv2.FILLED)
